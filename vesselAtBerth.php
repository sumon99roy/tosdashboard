<html>
	...
	<body>
		...
		<div align="center">
				<?php
				$strDate = isset($_POST['strDate']) ? $_POST['strDate'] : date('Y-m-d');
			...
			<br/>
                <tr style="margin:5px;">
                    <td align="center" colspan="7">
                        From Date: 
                        <input type="text" style="width:150px;" id="strDate" name="strDate" autocomplete="off" value="<?php echo $strDate; ?>" />
                        To Date: 
                        <input type="text" style="width:150px;" id="endDate" name="endDate" autocomplete="off" value="<?php echo $endDate; ?>" />
						<button type="submit" 
	...
	<br/>
		...
		<?php	if ($_SERVER['REQUEST_METHOD'] == 'POST')
					{
						$strDate=date_create($_POST['strDate']);
						...
						<table>
						<tr style="margin:5px;">
							<td colspan="12"><font size="5"><b>Berth Wise Vessel(Container) Operation at </b></font><font size="4"><?php echo $strDate;?> <b>To</b> <?php echo $endDate;?></font></td>
						</tr>
					</table>
					<?php } else {?>
				<table>
				...
				<?php
					...
					if ($_SERVER['REQUEST_METHOD'] == 'POST')
					{
						...
						$strQuery ="select  tbl3.*, dd || 'd ' || hh ||'h '|| mm ||'m' AS OCUPAI from(
							...
							to_char(argo_carrier_visit.atd,'dd/mm/yyyy hh24:mi:ss') as atd,
							to_char(argo_visit_details.etd,'dd/mm/yyyy hh24:mi:ss') as etd,
							CEIL(EXTRACT(HOUR FROM argo_visit_details.etd-argo_carrier_visit.ata)) as hh,
							CEIL(EXTRACT(DAY FROM argo_visit_details.etd-argo_carrier_visit.ata)) as dd,
							...
							INNER JOIN ref_bizunit_scoped ON ref_bizunit_scoped.gkey=vsl_vessel_visit_details.bizu_gkey
							WHERE argo_carrier_visit.phase IN ('30ARRIVED','40WORKING','60DEPARTED','70CLOSED') AND vsl_vessel_classes.basic_class='CELL' and argo_carrier_visit.ata 
							BETWEEN to_date(concat(to_char(cast($strDate as date),'yyyy-mm-dd'),' 00:00:00'),'yyyy-mm-dd hh24:mi:ss') and to_date(concat(to_char(cast($endDate as date),'yyyy-mm-dd'),' 23:59:59'),'yyyy-mm-dd hh24:mi:ss')
							ORDER BY argo_carrier_visit.phase,vsl_vessels.name
							) tb1)  tbl2)tbl3";
