   ...
   <tr>
	...
	$str = "SELECT equip_type,typ,tmd,dtmd,tot
		...
		(CASE 
			...
			WHEN tbl.equip_type LIKE 'Tele%' THEN 15
			WHEN tbl.equip_type LIKE 'Pipe%' THEN 16
			WHEN tbl.equip_type LIKE 'Automatic%' THEN 17
			WHEN tbl.equip_type LIKE 'Pneumatic%' THEN 18	
			WHEN tbl.equip_type LIKE 'Variable%' THEN 19			
			ELSE 20
		END			
...
<tr align="center">	
  ...
  <table border="1" align="center" cellspacing="1" cellpadding="1" width="80%">
	  <tr>
		<td colspan="8" align="center"><h2>Total Cargo Handling Equipment Position </h2></td>
	  </tr>	  
	  <tr>
		...
		<th rowspan="2">Others Workshop</th>
	  ...
	  <?php
	  $str2 = "SELECT IFNULL(equip_type,'Total=') AS equip_type,total,supply,stand_by,out_of_order, workshop,remarks
				FROM (
				SELECT equip_type,
				(CASE 
					...
					WHEN equip_type LIKE 'Tele%' THEN 15
					WHEN equip_type LIKE 'Pipe%' THEN 16
					WHEN equip_type LIKE 'Automatic%' THEN 17
					WHEN equip_type LIKE 'Pneumatic%' THEN 18			
					WHEN equip_type LIKE 'Variable%' THEN 19
					ELSE 20
					
				END
				) AS sl,
				(SELECT SUM(supply)+SUM(stand_by)+SUM(out_of_order)+SUM(workshop)) AS total,
				SUM(supply) AS supply,SUM(stand_by) AS stand_by, SUM(out_of_order) AS out_of_order,
				SUM(workshop) AS workshop,IFNULL(remarks,'') AS remarks
				FROM ctmsmis.equip_for_cargo_handling
				LEFT JOIN ctmsmis.equip_for_cargo_handling_remarks ON equip_for_cargo_handling.equip_type=equip_for_cargo_handling_remarks.equi_type
				 WHERE equip_for_cargo_handling.update_time=DATE(NOW()) GROUP BY equip_type WITH ROLLUP
				) AS tbl ORDER BY sl";
/* 				echo $str2;
				return; */
				
	$query2=mysqli_query($con_sparcsn4, $str2);					
...
<tr align="center">	
		...
		<?php if($row2->equip_type!="Total="){ ?>
		<td><?php if($row2->workshop) echo $row2->workshop; else echo "&nbsp;";?></td>
		<td style="width:300px;"><?php if($row2->remarks) echo $row2->remarks; else echo "&nbsp;";?></td>
		<?php } else { ?>
		<td><?php if($row2->workshop) echo $row2->workshop; else echo "&nbsp;";?></td>
		<td></td>
		<?php } ?>
